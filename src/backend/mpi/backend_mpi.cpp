...
void backend_mpi_t::initialize()
{
  ...
  LCW_Assert(provided == MPI_THREAD_MULTIPLE, "Cannot enable multithreaded MPI!\n");
...
int64_t backend_mpi_t::get_rank()
{
  return rank;
}

int64_t backend_mpi_t::get_nranks()
{
  return nranks;
}
...
bool backend_mpi_t::send(device_t device, rank_t rank, tag_t tag,
          ...
          comp_t completion, void *user_context)
{
  ...
  MPI_SAFECALL(MPI_Isend(buf, length, MPI_CHAR, rank, tag, device_p->comm_2sided, &entry.request));
  push_cq(completion, entry);
  return true;
}

bool backend_mpi_t::recv(device_t device, rank_t rank, tag_t tag,
          void *buf, int64_t length,
          comp_t completion, void *user_context)
{
  auto *device_p = reinterpret_cast<device_mpi_t*>(device);
  ...
  MPI_SAFECALL(MPI_Irecv(buf, length, MPI_CHAR, rank, tag, device_p->comm_2sided, &entry.request));
  push_cq(completion, entry);
  return true;
}

bool backend_mpi_t::put(device_t device, rank_t rank,
         void *buf, int64_t length,
         comp_t completion, void *user_context)
{
  auto *device_p = reinterpret_cast<device_mpi_t*>(device);
  ...
  MPI_SAFECALL(MPI_Isend(buf, length, MPI_CHAR, rank, -1, device_p->comm_2sided, &entry.request));
  push_cq(completion, entry);
  return true;
}
